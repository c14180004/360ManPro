...
if(isset($_POST['deleteGedungId'])){
    //delete gedung
    $deleteGedungId = $_POST['deleteGedungId'];
    $sqlDeleteGedung = "DELETE FROM gedung WHERE (id) IN (".$deleteGedungId.")";
    $resDeleteGedung = mysqli_query($conn,$sqlDeleteGedung);
    if($resDeleteGedung){
        echo "delete gedung success";
    }

    //ambil data tempat yang gedungnya di delete
    $arrDeleteGedungId = explode(",",$deleteGedungId);
    $arrIdTempat = [];
    $sqlDataTempat = "SELECT * FROM tempat";
    $resDataTempat = mysqli_query($conn,$sqlDataTempat);
    while($rowDataTempat = mysqli_fetch_assoc($resDataTempat)){
        if(in_array($rowDataTempat['id_gedung'],$arrDeleteGedungId)){
            array_push($arrIdTempat,$rowDataTempat['id']);
            unlink($rowDataTempat['image']);
        }
    }

    //delete tempat yang gedungnya hilang
    $strIdTempat = implode(",",$arrIdTempat);
